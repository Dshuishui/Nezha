...
type Config struct {
	...
	// 与这里的日志提交条件是两套独立机制。
	CommitQuorum string
	// BlockCacheMB 是 RocksDB 块缓存的大小（MiB），0 表示不开——**0 是历史默认**，
	// 而且它不是一个中性的默认值。
	//
	// 不开缓存时每一次点读都要从文件里读索引块、过滤块、数据块，四个系统都一样。
	// 那些读落在操作系统页缓存里，所以 10GB 规模下量不出代价，但它同时把
	// **KV 分离的结构优势一起抹掉了**：Nezha 的 LSM 只存 key→9 字节偏移，
	// Original 的要装完整的 value。有缓存时前者整个进得去、后者进不去——
	// 那才是"两跳打赢一跳"的机制。
	//
	// 这个字段存在的唯一目的就是把这件事测出来，所以默认保持 0：
	// 主表那 72 格是 0 跑的，改默认值会让新旧数据不可比。
	BlockCacheMB int

...
// 两处修正缺一不可：
//
//  1. 这里：只重写尾部覆盖到的分区。**实测下来这一条在本文的负载上没有生效**，
//     见 results/amplification/2026-09-09-writeamp：zipf 与 uniform、每一轮都是
//     「复用分区=0」。原先以为"Zipf 倾斜 ⇒ 尾部只碰少数分区"，这个推理是错的——
//     Zipf 倾斜的是每个 key 被写的**频次**，而决定一个分区要不要重写的是**覆盖**：
//     碰到一个 key 就得整块重写。尾部攒到 GC 阈值时已含数万个不同 key，
//     分区只有十来个，每个都必然被碰到，(1-1/12)^64000 ≈ 0。
//     选择性重写只在**真正的 key 区间局部性**下有价值（顺序键、时序写入）。
//     保留它是因为不花额外代价，而不是因为它在通用负载上省了什么。

...
| `three-node.sh` | both servers, `~/` | three-node control: `start IDX PORT IPORT VS N`, `kill9`, `restart` (same data directory, exercises recovery), `stop`, `report`, `recoverlog`; `BIN=race\|normal`; `SYSTEM=` picks the `-system` under test (default `nezha`); `EXTRA=` appends node flags; `GC_PAUSE_MS=` pauses GC after the file switch |

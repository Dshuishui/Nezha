...
say "    (this is the long step; GC should fire once the value log passes $GCGB GB)"
W=$(r tikv240 "source ~/env.sh; /tmp/randwrite -servers $ALL -cnums $CNUMS -dnums $N -vsize $VS")
echo "$W" | grep -vE 'new pool success' | tail -5 | sed 's/^/    /' | tee -a "$LOG"

...
sample)
  ...
  nohup bash -c '
    ...
    while [ -f "$D/sampler.pid" ]; do
      ...
      # **这一段整体在外层的单引号字符串里，所以里面一个单引号都不能出现，注释也不行。**
      # shell 的单引号字符串无法容纳单引号：写了就等于把字符串提前关掉，
      # 后面的内容交给外层 shell 解析，整段命令被拆坏。
      # 2026-09-18 连撞两次：先是空串判据写成 in <单引号><单引号>|... 被外层吃掉，
      # 只剩 in |*[!0-9]*) —— 采样循环语法错误、**一条数据都不写**，
      # 而外层文件的 bash -n 完全通过（错误在字符串里面，看不见）；
      # 接着在解释这件事的注释里又写了一对单引号，把字符串再次提前关掉。
      # 判空一律用 -z，不要用与空串比较的 case 分支。
      # 闸门自审第二十七节现在会把这一段抽出来单独做语法检查。
      [ -n "$datab" ] || datab=0

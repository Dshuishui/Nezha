...
watchdog() {
    ...
    for i in 0 1 2; do
        ...
        # **必须先确认这一行是数据，不是表头。** 采样器只写一次表头，之后才追加数据行；
        # 采样器若起不来（2026-09-18 真的发生过：单引号被吃掉导致整个循环语法错误），
        # `tail -1` 拿到的就是表头，而 `rss_kb` 这个字符串既不是空也不是 0——
        # 于是看门狗**拿一行表头当成健康的采样一路放行**，整段跑完什么都没监控到。
        # 判据改成"第一列必须是纯数字的时间戳"，表头的 `ts` 过不了。
        local ts0 rss
        ts0=$(scol "$s" 1)
        case "$ts0" in
            ''|*[!0-9]*)
                say "[中止] $label: node$i 的采样最后一行不是数据（拿到的是 '${ts0}'）——采样器没在写"
                bad=1; continue;;
        esac
        rss=$(scol "$s" 2)
...
say "===== 汇总 ====="
# **必须打出 system（第 5 列）。** 一个系统跑的时候不打也看得懂，四个系统一跑就是
# 48 行长得一模一样、认不出哪行是哪个系统的输出——而这一轮的交付物恰恰是
# "四个系统 × 三个操作的对比"，汇总认不出系统就等于没有汇总。
# 2026-09-18 的 smoke4b 就是这样：12 行里一个系统名都没有。
awk -F, 'NR>1{printf "%-3s %-11s %-7s v=%-5s p50=%-9s p99=%-9s ops/s=%-10s gc=%-3s lost=%-4s rss=%-8s fd=%-5s lag=%s\n", $3,$5,$11,$8,$14,$17,$24,$27,$28,$29,$30,$31}' "$OUT" | tee -a "$LOG"
if [ "$fail" = 0 ]; then
    say "全部通过"
else

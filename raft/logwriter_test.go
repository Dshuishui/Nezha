...
func TestAppendOffsetsAreContiguous(t *testing.T) {
	...
	rf.WriteEntryToFile(entries, rf.currentLog, 0)
...
func TestAppendOffsetsSurviveAcrossCalls(t *testing.T) {
	...
	rf.WriteEntryToFile([]*Entry{first}, rf.currentLog, 0)
	after := recordLen(rf, first)

	second := entryOf(2, "b", "22")
	rf.WriteEntryToFile([]*Entry{second}, rf.currentLog, 0)
...
func TestOverwriteThenAppendResumesAtEnd(t *testing.T) {
	...
	e1 := entryOf(1, "a", "AAAA")
	e2 := entryOf(2, "b", "BBBB")
	rf.WriteEntryToFile([]*Entry{e1, e2}, rf.currentLog, 0)
	sizeAfterTwo := recordLen(rf, e1) + recordLen(rf, e2)

	// 覆盖第二条
	rf.Offsets = rf.Offsets[:1]
	e2b := entryOf(2, "b", "CCCC")
	rf.WriteEntryToFile([]*Entry{e2b}, rf.currentLog, recordLen(rf, e1))
	...
	e3 := entryOf(3, "c", "DDDD")
	rf.WriteEntryToFile([]*Entry{e3}, rf.currentLog, 0)
...
func TestSwitchFileResetsOffset(t *testing.T) {
	...
	rf.WriteEntryToFile([]*Entry{entryOf(1, "a", "xxxx")}, rf.currentLog, 0)
	...
	rf.WriteEntryToFile([]*Entry{entryOf(2, "b", "yy")}, newLog, 0)

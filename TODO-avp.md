...
**进度（2026-09-04）**：A、B、C、D、F 已完成（`395a30a` `31a817d` `d39ed6d`），G 的 `os.SEEK_*`
与 H 里最刺眼的几条已改（`2a4829d`）；E 按用户决定全部保留；剩余：G 的 `grpc.Dial` 族、I。

J. **文档与注释全面英文化**（用户 2026-09-04 要求：文件内容以英文为主，走专业开发流程）。
   今天新增的文档与注释已是英文；存量代码里的中文注释（raft.go / FlexSync.go 各数百行）与
   日志字符串待翻。注意 `scripts/multinode/*.sh` 用 `垃圾回收完成` `建立了索引` `3秒没有收到`
   等日志串做判定，改日志时要同步改脚本。`TODO-avp.md` 本身作为工作日志暂保留中文，
   后续可拆成英文的 `docs/development-log.md`。

**建议顺序（剩余）**：J → G（grpc.NewClient，需验证 WithBlock 语义差异）→ I。

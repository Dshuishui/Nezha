...
**未验证（不可宣称"多节点完全可靠"）**：
- [ ] **只跑了一次**。竞态与时序相关，需多轮 + 多个 value 大小
- [ ] **冲突覆盖路径未触发**。`offsetVersions` 的同步截断没被测到，
      要杀 leader 制造选举才走得到
- [ ] **故障切换后的读未测**。这个 bug 最严重的后果是"follower 当选后提供脏数据"，
      而目前只验证了"follower 的 GC 不报错"
- [ ] **三节点未测**。论文口径是 3 副本，现有两台

**下一步**：故障切换验证 —— 写入 → 杀 leader → 等 follower 当选 → 从新 leader
读并校验完整性。一次覆盖上面第 2、3 项。
